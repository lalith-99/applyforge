...
type ClaimSubmissionIntentForUserParams struct {
	ID       pgtype.UUID
	UserID   pgtype.UUID
	WorkerID string
}

const claimSubmissionIntentForUser = `
WITH expired AS (
...
func (q *Queries) ClaimSubmissionIntentForUser(ctx context.Context, arg ClaimSubmissionIntentForUserParams) (SubmissionIntent, error) {
	return scanSubmissionIntent(q.db.QueryRow(ctx, claimSubmissionIntentForUser, arg.ID, arg.UserID, arg.WorkerID))
}
