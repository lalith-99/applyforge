...
// SetOnChanged registers a callback fired after preferences are updated
// (e.g. to recompute precomputed job recommendations, since preferences
// affect matching hard filters). Returns the same Handlers for chaining.
...
func toResponse(p Preferences) map[string]any {
	return map[string]any{
		...
		"hybrid":                                 p.Hybrid,
		"onsite":                                 p.Onsite,
		"preferred_locations":                    orEmpty(p.PreferredLocations),
		"willingness_to_relocate":                p.WillingnessToRelocate,
		"employment_types":                       orEmpty(p.EmploymentTypes),
		"minimum_salary":                         p.MinimumSalary,
		"excluded_companies":                     orEmpty(p.ExcludedCompanies),
		"excluded_locations":                     orEmpty(p.ExcludedLocations),
		"excluded_industries":                    orEmpty(p.ExcludedIndustries),
