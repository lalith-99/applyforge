...
func (w *ComputeWorker) Handle(ctx context.Context, job background.Job) error {
	...
	if len(freshCandidates) == 0 {
		slog.Info("daily recommendation candidates rejected before AI reranking", "user_id", userID, "candidate_count", len(candidates), "fresh_candidate_count", 0)
		return w.repo.ReplaceForUser(ctx, userID, nil)
	}
	...
	slog.Info("daily recommendation shortlist computed", "user_id", userID, "candidate_count", len(candidates), "fresh_candidate_count", len(freshCandidates), "ranked_count", len(ranked), "recommendation_count", len(recs), "candidate_profile_version", version)
...
func toRecommendations(ranked []airank.RankedJob, version int32) []Recommendation {
	...
	for _, r := range ranked {
		...
		rec := Recommendation{JobID: r.Job.ID, DeterministicScore: int32(r.Result.TotalScore), FinalScore: int32(finalScore), CandidateProfileVersion: &version, ImmigrationStatus: r.Result.Eligibility.Immigration.Status, ImmigrationConfidence: r.Result.Eligibility.Immigration.Confidence, ImmigrationEvidence: r.Result.Eligibility.Immigration.Evidence, ImmigrationPriorityScore: int32(r.Result.ImmigrationPriorityScore)}
		...
		if r.HasJudgment {
			...
			strongCandidate = strongCandidate && (recommendation == "APPLY_NOW" || recommendation == "STRONG_CONSIDER")
		} else {
			strongCandidate = strongCandidate && r.Result.TotalScore >= MinStrongDailyRecommendationScore
		}
		candidate := dailyRecommendationCandidate{recommendation: rec, companyID: r.Job.CompanyID}
		if strongCandidate {
	...
	recs = appendDiverseRecommendations(recs, strong, companyCounts, DailyRecommendationLimit)
	if len(recs) < DailyRecommendationTargetMinimum {
		recs = appendDiverseRecommendations(recs, backfill, companyCounts, DailyRecommendationTargetMinimum)
	}
	return recs
}

func appendDiverseRecommendations(dst []Recommendation, candidates []dailyRecommendationCandidate, companyCounts map[uuid.UUID]int, limit int) []Recommendation {
	for _, candidate := range candidates {
		if len(dst) >= limit {
			break
		}
...
func dailyPriorityScore(r airank.RankedJob, freshness int) int {
	...
	if r.Result.ImmigrationRelevant {
		score = 0.30*float64(fit) + 0.20*float64(r.Result.TotalScore) + 0.15*float64(career) + 0.10*float64(interview) + 0.10*float64(freshness) + 0.15*float64(r.Result.ImmigrationPriorityScore)
	} else {
		score = 0.35*float64(fit) + 0.25*float64(r.Result.TotalScore) + 0.15*float64(career) + 0.10*float64(interview) + 0.15*float64(freshness)
	}
