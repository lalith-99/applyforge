...
export default function ApplicationsPage() {
  ...
  const updateStatus = useMutation({
    ...
    onMutate: async ({ id, status }) => {
      ...
      queryClient.setQueryData<ApplicationWithJob[]>(["applications"], (current = []) =>
        current.map((app) =>
          app.ID === id ? { ...app, Status: status, UpdatedAt: new Date().toISOString() } : app,
        ),
      );
      setRecentlyMoved(id);
      return { previous };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      document.querySelector(`[data-application-id="${recentlyMoved}"]`)?.scrollIntoView({
        behavior: "smooth",
        block: "nearest",
        inline: "center",
      });
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-black/55 dark:text-white/55">
              Review the exact resume and answers before authorizing any application submission.
            </p>
          </div>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-black/10 text-xs uppercase tracking-wide text-black/50 dark:border-white/15 dark:text-white/50">
                  <th className="py-2">Company</th><th className="py-2">Title</th><th className="py-2">Status</th><th className="py-2">Match</th><th className="py-2">Updated</th><th className="py-2">Action</th>
                </tr>
              </thead>
              <tbody>
...
function ApplicationCard({ app, onAdvance, onReview, highlighted, updating }: {
  ...
  return (
    <div data-application-id={app.ID} className={`flex flex-col gap-2 rounded-md border p-3 text-sm transition-all dark:border-white/15 ${highlighted ? "border-blue-400 bg-blue-50 ring-2 ring-blue-200 dark:bg-blue-950/20" : "border-black/10"}`}>
